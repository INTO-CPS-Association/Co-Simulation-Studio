...
export function getCosimPath(
    scope: vscode.TextDocument | vscode.WorkspaceFolder
) {
    const cosimPath = vscode.workspace
        .getConfiguration('cosimstudio', scope)
        .get('cosimPath')
...
export function isDocumentInWorkspace(document: vscode.TextDocument): boolean {
    ...
    return workspaceFolders.some((folder) =>
        documentUri.startsWith(folder.uri.toString())
    )
}
...
export function isDocumentCosimConfig(document: vscode.TextDocument) {
    return (
        ...
        document.uri.path.endsWith(`/${getCosimPath(document)}`) &&
        isDocumentInWorkspace(document)
    )
}
