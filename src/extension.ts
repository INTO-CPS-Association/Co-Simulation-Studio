...
function registerCosimConfigTracking(context: vscode.ExtensionContext) {
    ...
    const disposable = vscode.window.onDidChangeActiveTextEditor(
        (newActiveEditor) => {
            if (
                !newActiveEditor ||
                !isDocumentCosimConfig(newActiveEditor.document)
            ) {
                vscode.commands.executeCommand(
                    'setContext',
                    'cosimstudio.cosimConfigOpen',
            ...
            extensionLogger.info(
                `Opened cosim config file: ${newActiveEditor.document.uri.path}`
            )
        }
    )
...
function registerProviders(
    ...
    const cosimRelativePattern = new vscode.RelativePattern(
        wsFolder,
        getCosimPath(wsFolder)
    )
