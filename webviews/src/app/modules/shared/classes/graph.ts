...
export class Graph {
    ...
    public getDataset(graph: LiveGraph): BehaviorSubject<Array<any>> | undefined {
    ...
    public initializeSingleDataset(g: LiveGraph) {
        this.graphMap.clear();
        let ds = new BehaviorSubject<any[]>([]);
    ...
    public initializeDatasets() {
        ...
        this.config?.liveGraphs.forEach((g: any) => {
    ...
    public launchWebSocket(webSocket: string) {
        ...
        if (this.webSocketOnOpenCallback != null)
    ...
    private onMessage(event: MessageEvent) {
        ...
        if ("time" in rawData) {
            ...
            if (this.progressCallback) {
                if (rawData.time < (this.config?.endTime ?? -1)) {
                    let pct = (rawData.time / (this.config?.endTime ?? -1)) * 100;
