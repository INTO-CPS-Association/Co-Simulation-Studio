...
export class Fmu {
	...
	private populateFromModelDescription(content: string) {
		...
		while (thisNode) {
			...
			var type: ScalarVariableType | undefined;
			...
			let causality: CausalityType | undefined;
			...
			let variability: VariabilityType | undefined;
			...
			let initial: InitialType | undefined;
			...
			if (sv != null) {
				sv.type = type ?? sv.type;
				sv.causality = causality ?? sv.causality;
				sv.isConfirmed = true;
				sv.variability = variability ?? sv.variability;
				sv.start = start ?? sv.start;
				sv.initial = initial ?? sv.initial;
			}
...
export class ScalarVariable {
	constructor(
		...
		public start: string | undefined = undefined,
