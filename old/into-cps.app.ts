...
// constants
const topBarNameId = "activeTabTitle";
 ...
 const globalAny:any = global;
    ...
    constructor(app: Electron.App, processPlatform: string) {
        ...
        const defaultValues = SettingKeys.DEFAULT_VALUES;
        const projRoot = Utilities.getSystemPlatform() == "windows" ? this.app.getPath('documents') : this.app.getPath('home');
        const defaultProjectFolderPath = Path.join(projRoot, "into-cps-projects");
        ...
        for (const key in defaultValues) {
            if (this.settings.getSetting(key) == null) {
                const value: any = SettingKeys.DEFAULT_VALUES[key];
    ...
    public loadPreviousActiveProject(){
        const activeProjectPath = this.settings.getSetting(SettingKeys.ACTIVE_PROJECT);
    ...
    public createProject(name: string, path: string) {
        const project = new Project(name, path, Path.normalize(path + "/.project.json"));
        project.save();
    ...
    loadProject(path: string): IProject {
        ...
        const config = Path.normalize(path);
        const content = fs.readFileSync(config, "utf8");
        // TODO load configuration containers and config files
        const project = SerializationHelper.toInstance(new Project("", "", ""), content.toString());
    ...
    public static setTopName(s: string) {
        const mainName = (<HTMLSpanElement>document.getElementById(topBarNameId));
