...
export class CoeProcess {
  ...
  private getWorkingDir() {
    const installDir = this.settings.getValue(SettingKeys.INSTALL_TMP_DIR);
    const childCwd = Path.join(installDir, "coe-working-dir");
  ...
  public getPid(): string {
    if (fs.existsSync(this.getPidFilePath())) {
      //this.stop();
      const pid = fs.readFileSync(this.getPidFilePath());
      return pid + "";
    }
    return null;
  }
  ...
  public getCoePath() {
    ...
    const overrideCoePath = this.settings.getValue(SettingKeys.COE_JAR_PATH);
  ...
  public start() {
    ...
    const childCwd = this.getWorkingDir();
    ...
    const child = spawn("java", ["-jar", this.getCoePath()], {
      detached: true,
      shell: false,
      cwd: childCwd,
      env: env
  ...
  public subscribeLog4J(callback: any) {
    const logFile: string = this.getLog4JFilePath();
